                ...
                <div class="modal-dialog modal-dialog-centered" role="document">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="multipurpose_model_label">Add Student</h5>
                        ...
                        <div class="modal-body">
                            ...
                            <input type="text" class="form-control btn-small mb-2" name="stud_no" id="stud_no" placeholder="Student No">
                            <label class="sr-only" for="stud_name">Student Name</label>
                            <input type="text" class="form-control btn-small" name="stud_name" id="stud_name" placeholder="Student Name">
                        </div>
            ...
            <style>
                ...
                <script>
                    function clean_modal() {
                        $('#stud_no').val('').removeAttr('disabled');
                        $('#stud_name').val('');
                        $('#multipurpose_model_label').text('Add Student');
                        $('#add_btn').show();
                        $('#save_btn').hide();
                    }
                    function add_info() {
                        ...
                        $.ajax({
                            ...
                            dataType: 'json',
                            data: {type: 'add', no: stud_no, name: stud_name},
                            beforeSend: function () {},
                            success: function (response) {
                                if(response.success) {
//                                     $('#info_table').append('<tr id="info_row_'+stud_no+'">\
//     <td>\
//       <a class="align-items-center text-danger mr-2 small d-inline-flex" href="javascript:delete_info(\''+stud_no+'\')" title="Delete"><span data-feather="x"></span> Delete</a>\
//       <a class="align-items-center text-info small d-inline-flex" href="javascript:edit_info(\''+stud_no+'\')" title="Edit"><span data-feather="edit"></span> Edit</a>\
//     </td>\
//     <td id="info_no_'+stud_no+'">'+stud_no+'</td>\
//     <td id="info_name_'+stud_no+'">'+stud_name+'</td>\
// </tr>');
//                                     feather.replace();
//                                     $('#multipurpose_model').modal('hide');
                                    window.location.reload();
                                } else {
                                    alert('Make sure student no begins with `C`');
                                }
                            },
                            error: function () {
                    ...
                    function edit_info(index) {
                        $('#multipurpose_model_label').text('Edit Student');
                        $('#stud_no').val($('#info_no_'+index).text()).attr('disabled', 'disabled');
                        $('#stud_name').val($('#info_name_'+index).text());
                        $('#save_btn').show().attr('onclick', "edit_info_action('"+index+"')");
                        $('#add_btn').hide();
                        $('#multipurpose_model').modal('show');
                    }
                    function edit_info_action(stud_no) {
                        let stud_name = $('#stud_name').val();
                        $.ajax({
                            method: 'GET',
                            url: './ajax/student.php',
                            dataType: 'json',
                            data: {type: 'update', no: stud_no, name: stud_name},
                            beforeSend: function () {},
                            success: function (response) {
                                if(response.success) {
                                    // $('#info_name_'+stud_no).text(stud_name);
                                    // $('#multipurpose_model').modal('hide');
                                    window.location.reload();
                                } else {
                                    alert('An unexpected error occurred!');
                    ...
                    function delete_info(stud_no) {
                        $.ajax({
                            method: 'GET',
                            url: './ajax/student.php',
                            dataType: 'json',
                            data: {type: 'delete', no: stud_no},
                            beforeSend: function () {},
                            success: function (response) {
                                if(response.success) {
                                    // $('#info_row_'+stud_no).remove();
                                    window.location.reload();
                                } else {
                                    alert('An unexpected error occurred!');
                                }
